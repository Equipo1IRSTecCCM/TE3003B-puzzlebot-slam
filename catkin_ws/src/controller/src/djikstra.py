...
def get_neighbors(mat,r,c):
    ...
    try:
        if r > 0 and c > 0 and not mat[r-1][c-1].processed:
                neighbors.append(mat[r-1][c-1])
        if r < shape[0] and c > 0 and not mat[r+1][c-1].processed:
                neighbors.append(mat[r+1][c-1])
        if r > 0 and c < shape[1] and not mat[r-1][c+1].processed:
                neighbors.append(mat[r-1][c+1])
        if r < shape[0] and c < shape[1] and not mat[r+1][c+1].processed:
                neighbors.append(mat[r+1][c+1])
    except:
        pass
...
def get_distance(img,u,v):
    ...
    a = (float(img[v[0],v[1]])-float(img[u[0],u[1]]))**2
    # b = (float(img[v][1])-float(img[u][1]))**2
    # c = (float(img[v][2])-float(img[u][2]))**2
    return 0.1 + a * 3
