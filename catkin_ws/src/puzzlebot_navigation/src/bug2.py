...
def callbackScan(msg):
    ...
    nfb = not scan_f[scan_f < 0.7].shape[0] > 0 and not scan_l[scan_l < 0.65].shape[0] > 0
    ilb = scan_l[scan_l < 0.7].shape[0] > 0

    #Calculate slope with regression and give it as error
    if scan_l[scan_l < 0.7].shape[0] >= 13:
